...
### 坑 27：分享海报视觉单薄，且长文字溢出画布

**现象**：早期海报只有一个大数字加几行小字，视觉单薄；当活动名、子标题或指标值较长时，文字直接跑到画布外（被裁切或叠到边框上）。

**原因**：Canvas `fillText` 不会自动换行或缩放，旧版代码直接用固定字号并以固定坐标绘制，长文本必然溢出。

**解决**：抽出统一的海报视觉系统 `POSTER_UI`（暗底绿渐变 + 网格纹理 + 品牌头）与万能绘制函数 `renderSigmaPoster`：新增 `_fitFont()`（自动缩小字号直到适容）与 `_ellipsis()`（超长截断加省略号），所有文本都先量宽再绘制。海报统一为 680×1000，底部固定“由 PROATHLETE 生成”与时间水印。

**教训**：任何 Canvas 文字都要先 `measureText` 再决定字号/截断，不能假设“字符串不会太长”。

### 坑 28：分享数据“不说人话”

**现象**：只给“772.1 公里”、“9000 米爬升”这种硬数字，圈外人看不出厉害，分享没有话题感。

**解决**：新增 `funComparisons()` 把累计值换算成生活化对比——“相当于 北京→上海 N 次”、“绕上海外环 N 圈”、“全程马拉松 N 个”、“累计爬升≈珠峰 N 座”、“消耗≈米饭 N 碗 / 汉堡 N 个”。常量集中在 `FUN`（北京-上海 1080km、上海外环 99km、马拉松 42.195km、上海中心大厦 632m、珠峰 8848.86m、米饭 200kcal/碗、汉堡 500kcal/个），汇总与单次海报均自动带上「相当于」区块。

**教训**：分享要服务“被转发”，得把数据译成大众能感知的对比量。

### 坑 29：单次详情只有大轨迹图，缺“一眼看懂”的缩略图

**现象**：打开单次辐迹详情时，轨迹只在下方大图里，指标区旁边空荡，不能边看数据边看路径。

**解决**：为详情弹窗加 `.a-hero`（指标网格 + 轨迹缩略并排），`openActivityDetail()` 里复用 `drawTrackMini()` 将本次轨迹缩略绘制于 #aTrackMini，无 GPS 时自动隐藏；小屏时纵向堆叠。

**教训**：详情页要“数据 + 空间”同屏，让用户一眼把距离和路径对上。

### 坑 30：热力图与生涯 PB 无法分享

**现象**：年度热力图与个人最佳记录是最有“数据成就感”的两块，却没有分享入口。

**解决**：新增 `renderHeatmapPoster()`（将实时 `#heatmapCanvas` 按比例缩放绘入海报 + 累计距离/次数/时长三格 + 相当于）与 `renderPBPoster()`（按运动类型罗列 PB 瓦片，带类型色条与日期），分别挂到热力图卡片与个人最佳区的“分享”按钮（#btnShareHeatmap / #btnSharePB）。

**教训**：凡是用户愿意晒的视图，都应配一个一键分享。
...
| v2.8 | **分享海报重制 · 趣味对比 · 轨迹缩略 · 热力图/PB 分享**：① 海报全面重制为 Sigma 风（暗底绿渐变、大字累计值、轨迹剪影网格、底部「相当于」对比），`_fitFont`/`_ellipsis` 修复文字溢出；② 新增趣味对比（北京-上海/绕上海/马拉松/珠峰/米饭碗 等）；③ 运动详情页旁新增本次轨迹缩略图；④ 新增热力图分享与生涯 PB 分享 |
...
| 区块 | 主要变化 |
|---|---|
| **功能特性 · 分享** | 分享海报全面重制为 **Sigma 风**：暗底绿渐变、大字累计值、轨迹剪影网格、底部「相当于」对比；`_fitFont`/`_ellipsis` 修复长文字溢出；新增 **热力图分享** 与 **生涯 PB 分享** |
| **趣味对比** | 新增 `funComparisons()`：北京→上海 N 次、绕上海外环 N 圈、全程马拉松 N 个、累计爬升≈珠峰 N 座、≈米饭 N 碗 / 汉堡 N 个 |
| **运动详情** | 详情弹窗新增 **本次轨迹缩略图**（`.a-hero` + `#aTrackMini`），与指标网格并排，无 GPS 自动隐藏 |
| **版本演进** | 新增 v2.8 |
| **踩坑记录** | 新增 4 条：坑 27 海报视觉单薄 + 文字溢出、坑 28 分享数据不说人话、坑 29 详情缺轨迹缩略、坑 30 热力图/PB 无法分享 |
