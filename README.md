...
### 坑 24：大天数同步（如 365 天）失败或只同步了最近一天

**现象**：同步范围拉到 365 天时，要么直接失败，要么只回填了最近一天的数据。

**原因**：两处叠加——① 前端把增量同步写成 `days_back: lastSync ? 0 : daysBack`，一旦有过同步记录，请求范围就被压成「最近一天」；② 后端 `get_activities_by_date(start, end)` 一次性拉取整段（一年）活动列表，数据量大时容易超时/被截断，于是只剩回退的少量结果。此外前端轮询超时仅 20 分钟，长任务被过早判定失败。

**解决**：前端始终按用户配置的天数请求（`days_back: daysBack`），轮询超时放宽到 90 分钟；后端把区间按 **60 天分块**逐段拉取、按 `activityId` 去重，并在 `days_back > 0` 时以配置天数为准。

**教训**：大范围抓取要分块 + 去重 + 让步重试，前端超时阈值要和后端实际耗时匹配。

---

### 坑 25：FIT 无效坐标哨兵导致跑步「室内/室外」判错

**现象**：室内跑步（跑步机、无 GPS）被识别成室外跑，总览里「室外/室内」卡片数据明显不对。

**原因**：FIT 里无效坐标用哨兵值 `0x7FFFFFFF`（21 亿+）表示。解码时直接 `v * (180 / 2^31)`，哨兵被换算成一个「看似合法的纬度」，于是本无 GPS 的轨迹点被计为有 GPS，`hasGPS` 判定为真 → 误判室外。

**解决**：解码前先过滤哨兵（`0x7FFFFFFF` / `-2^31`），无坐标即视为无 GPS；并抽出统一的 `detectIndoor(type, points, subSport, rawText)`：文本线索（室内/跑步机/骑行台/泳池…）→ FIT `subSport`（跑步机=1、室内跑=9、室内骑行=5…）→ 轨迹 GPS 判定，三挡兜底；FIT / GPX / TCX / XLSX / Garmin CSV 全部接入同一判定。

**教训**：二进制格式里的「无效值哨兵」必须先过滤再用，否则会被当成真实数据参与判断。

---

### 坑 26：分享数据太单一，只有一张总览海报

**现象**：分享只能发「近 7 天总览」，用户想分享单个指标或某张卡片时无从下手。

**解决**：抽出通用海报渲染（`_posterBg` / `renderTextPoster` / `renderSummaryPoster` / `renderActivityPoster` / `renderOverviewPoster`），每个数据卡片右下角注入 `.fig-share` 按钮，鼠标悬停浮现、点击即生成该卡片的海报；总览页另加「汇总分享」入口，一次性汇总全部运动的累计距离/时长/次数与各类型分布。

**教训**：分享的颗粒度应跟随信息的颗粒度——既要「总」，也要能「单点」。
...
| v2.7 | **筛选 · 备份 · 分享 · 同步修复**：① 各运动按类型距离/条件筛选（跑步 5/10/15/半马/全马/超马 等）；② Garmin 同步后原始数据自动备份，支持 ZIP 原始备份 / 活动 CSV / 全量 JSON 导出与 JSON 备份导入；③ 分享海报支持各卡片单独分享 + 「全部运动汇总」汇总分享；④ 修复大天数（365 天）同步失败/仅同步一天；⑤ 上传数据可区分室内/室外跑步 |
...
| 区块 | 主要变化 |
|---|---|
| **功能特性 · 运动列表** | 新增「按类型距离/条件筛选」：跑步 5/10/15/半马/全马/超马，骑行 20/40/80/120K，游泳 500/1000/1500/3000m，徒步、步行、力量按运动合理分档 |
| **功能特性 · 备份导出** | 新增「原始数据备份与导出」：Garmin 同步/文件导入时原始文件自动落库，支持 ZIP 原始备份、活动 CSV、全量 JSON 导出与 JSON 备份导入 |
| **功能特性 · 分享** | 分享海报支持**各卡片单独分享**（悬停出现分享按钮）与**「全部运动汇总」汇总分享** |
| **Garmin 同步** | 修复大天数（365 天）同步失败/仅同步一天：前端按配置天数请求、超时 20→90 分钟；后端 60 天分块拉取 + `activityId` 去重 |
| **室内/室外** | 修复跑步室内外误判：过滤 FIT 无效坐标哨兵，统一 `detectIndoor` 判定并接入全部导入格式 |
| **版本演进** | 新增 v2.7 |
| **踩坑记录** | 新增 3 条：坑 24 大天数同步失败、坑 25 FIT 哨兵导致室内外误判、坑 26 分享颗粒度 |
